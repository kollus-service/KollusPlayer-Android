...
public class JwtUtil {
    public String createJwt(final String headerJson, final String payloadJson, String secretKey)
        ...
        String header = Base64.encodeToString(headerJson.getBytes(StandardCharsets.UTF_8), Base64.URL_SAFE);
        String payload = Base64.encodeToString(payloadJson.getBytes(StandardCharsets.UTF_8), Base64.URL_SAFE);
        String content = String.format("%s.%s", header, payload);
        final Mac mac = Mac.getInstance("HmacSHA256");
        mac.init(new SecretKeySpec(secretKey.getBytes(StandardCharsets.UTF_8), "HmacSHA256"));
        byte[] signatureBytes = mac.doFinal(content.getBytes(StandardCharsets.UTF_8));
        String signature = Base64.encodeToString(signatureBytes, Base64.URL_SAFE);
